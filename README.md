...
> **WebSocket 범위:** 이 스킬은 로그인하지 않은 공개 페이지에 표시되는 실시간 체결가 갱신을 브라우저에서 관찰하고 정제해 설명하는 범위만 지원합니다. WebSocket 연결에는 임시 게스트 연결 메타데이터가 필요하므로, 원본 게스트 값이나 원본 프레임의 요청·출력·저장·기록·재생 및 독립 WebSocket 클라이언트 제공·구현은 지원하지 않습니다. WebSocket 매수·매도 호가 구독과 모든 주문·계좌 작업도 제외됩니다. 현재가·호가 스냅샷·장중 체결 틱은 기존 `scripts/quote.py`의 제한된 공개 HTTP 읽기 전용 조회로 계속 지원합니다. 자세한 경계는 [WebSocket 관찰 노트](references/websocket-observations.md)를 참고하세요.
