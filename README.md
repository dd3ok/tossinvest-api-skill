...
이 스킬은 `developers.tossinvest.com/docs`의 토스증권 공식 Open API 클라이언트가 아닙니다. 토스증권 공개 웹 페이지에 이미 표시되는 주식·시장 데이터를 다시 조회하도록 돕는 경량 에이전트 스킬입니다. 그래서 공식 Open API 앱 설정, OAuth 토큰, 계좌 헤더, IP 등록 절차가 필요하지 않습니다.

공식 Open API 연동이나 계좌·자산·주문 업무가 필요하다면 이 저장소의 스크립트를 확장하지 말고 공식 문서를 기준으로 별도 클라이언트를 구현하세요.
...
- `dashboard_ranking.py --kind signals`로 홈 실시간 차트의 AI 요약 라벨 필드를 조회할 수 있습니다. 이 값은 공개 UI 라벨일 뿐 매수·매도 신호가 아닙니다.
- `trading_trend.py`로 신용, 대차거래, 공매도 추이, CFD 같은 거래 현황 페이지의 공개 데이터를 조회할 수 있습니다. 계좌 한도, 주문 가능 여부, 레버리지 판단, 투자 조언으로 쓰지 않습니다.
- `VWAP.KRW-BTC` 같은 암호화폐형 지수 페이지를 지원합니다. 관련 경로에는 `r-chart/crypto`와 `/api/v1/crypto-prices`가 포함됩니다.
- `/calendar`의 월간 이벤트, 경제지표, 실적 발표일, 국내·해외 필터, 주요 이벤트, 주간 AI 요약 문구를 확인할 수 있습니다. 이 문구 역시 투자 조언이나 매수·매도 신호가 아닙니다.
- 공개 경제지표 상세 페이지와 지수 페이지의 캘린더 일부 데이터를 정확히 허용된 `wts-cert-api` 경로와 쿼리로만 조회합니다.
- 피드·뉴스 라벨은 주요 뉴스(`ALL_HIGHLIGHT`), 최신 뉴스(`HOT`), 급등주 뉴스(`SOARING_STOCK`), 지수 뉴스(`INDEX`)를 기준으로 정리합니다.
- 스크리너는 `C_주가등락률_1W` 가격 변화 정렬 컬럼을 포함한 현재 공개 프리셋을 기준으로 다룹니다.
...
설치 후에는 Codex를 재시작해 스킬 목록을 다시 로드하세요. 이후 TossInvest/토스증권을 언급한 주식 데이터 요청이 이 스킬로 연결됩니다.
...
이미 클론한 작업 디렉터리를 쓰고 싶다면 심볼릭 링크로 노출할 수도 있습니다.
...
특정 저장소 안에서만 쓰고 싶다면 이 저장소를 아래 위치에 복사하거나 벤더 디렉터리처럼 포함하세요.
...
Claude는 TossInvest/토스증권 공개 주식·시장 데이터 요청이 지원 범위와 맞으면 이 스킬을 사용할 수 있습니다.

### Gemini CLI

Gemini CLI 확장으로 설치할 수 있도록 `gemini-extension.json`과 `GEMINI.md`를 포함합니다. Gemini CLI는 설치한 확장을 복사하므로, GitHub나 로컬 경로에서 설치한 뒤 변경사항을 반영하려면 `gemini extensions update`가 필요합니다.
...
스크립트별 옵션은 `--help`로 확인합니다. 미국 주식 차트는 TossInvest 상품/소스 코드가 필요합니다. `SPY`, `NVDA` 같은 표시 티커를 `c-chart` 상품 코드로 바로 넣으면 HTTP 400이 날 수 있습니다.
