...
- 종목 요약, 공개 HTTP 현재가·호가 스냅샷·장중 체결 틱 조회
...
> **WebSocket API 범위:** 로그인하지 않은 공개 페이지에 표시되는 실시간 체결가를 서버·STOMP 수명주기·구독 destination·반복 `MESSAGE` 이벤트·payload 필드 형식으로 설명합니다. 연결에는 임시 게스트 연결 메타데이터가 필요하므로 원본 값이나 프레임의 요청·출력·저장·기록·재생, 독립 WebSocket 클라이언트 제공·구현은 지원하지 않습니다. WebSocket 매수·매도 호가 구독과 모든 주문·계좌 작업도 제외됩니다. 현재가·호가 스냅샷·장중 체결 틱은 기존 `scripts/quote.py`의 제한된 공개 HTTP 읽기 전용 조회로 계속 지원합니다. 자세한 내용은 [비공식 WebSocket API 레퍼런스](references/websocket-api-reference.md)를 참고하세요.
