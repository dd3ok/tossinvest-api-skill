...
Codex는 파일시스템의 skill 폴더를 자동으로 탐색합니다. 개인용으로 설치하려면 저장소를 clone한 뒤 `tossinvest-web-api` 폴더를 `~/.agents/skills` 아래에 노출하세요.

```bash
mkdir -p ~/.agents/skills
git clone https://github.com/dd3ok/tossinvest-api-skills.git ~/src/tossinvest-api-skills
ln -sfn ~/src/tossinvest-api-skills/tossinvest-web-api ~/.agents/skills/tossinvest-web-api
```

skill이 바로 보이지 않으면 Codex를 재시작하세요. 이후 프롬프트에서 명시적으로 호출할 수 있습니다.

```text
Use $tossinvest-web-api to get a compact stock summary and current quote for A005930.
```

특정 저장소 안에서만 쓰고 싶다면 이 폴더를 아래 위치에 복사하거나 vendor 형태로 포함하세요.
...
ln -sfn /path/to/tossinvest-api-skills/tossinvest-web-api ~/.claude/skills/tossinvest-web-api
```
...
```bash
mkdir -p .claude/skills
cp -R /path/to/tossinvest-api-skills/tossinvest-web-api .claude/skills/tossinvest-web-api
```
